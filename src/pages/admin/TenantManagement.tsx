...
export interface Tenant {
  ...
  eNachStatus: 'active' | 'pending' | 'rejected' | 'not_registered';
...
const TenantManagement: React.FC = () => {
  ...
  const filteredTenants = tenants.filter((tenant) => {
    ...
    const matchesStatus =
      statusFilter === 'all' || tenant.eNachStatus === statusFilter;
  ...
  const columns = [
      ...
      render: (tenant: Tenant) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {tenant.propertyLocation.split(',')[0]}
          </p>
        </div>
      )
    },
    ...
    },
    {
      ...
      render: (tenant: Tenant) => (
        <Badge
          variant={
            tenant.eNachStatus === 'active'
              ? 'success'
              : tenant.eNachStatus === 'pending'
              ? 'warning'
              : tenant.eNachStatus === 'rejected'
              ? 'destructive'
              : 'inactive'
          }
        >
          {tenant.eNachStatus === 'not_registered'
            ? 'Not Registered'
            : tenant.eNachStatus}
        </Badge>
      )
        ...
        <Badge
          variant={
            tenant.contractStatus === 'active'
              ? 'success'
              : tenant.contractStatus === 'expiring_soon'
              ? 'warning'
              : 'destructive'
          }
        >
